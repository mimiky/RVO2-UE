...
#pragma omp parallel for
#endif /* _OPENMP */
  ParallelFor(agents_.size(), [this](int32 i)->void{
    this->agents_[i]->computeNeighbors(kdTree_);
    this->agents_[i]->computeNewVelocity(timeStep_);
  });
...
const Vector2& RVOSimulator::getAgentOldPosition(std::size_t agentNo) const
{
  return agents_[agentNo]->oldPosition_;
}
